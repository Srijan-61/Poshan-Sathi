...
export default function AddedIngredientsList({
  addedList,
  setAddedList
}: AddedIngredientsListProps) {  const heading = "text-neutral-900";
  ...
  return (
    <div className="mt-8">
      ...
      {addedList.length === 0 ? (
        ...
        >
          <span
            className={`material-symbols-outlined text-5xl opacity-80 ${"text-green-600/60"}`}
            aria-hidden
          >
            soup_kitchen
          </span>
          ...
          {addedList.map((item, index) => (
            ...
            >
              ...
              <div className="min-w-0 flex-1 flex flex-col justify-center">
                <p className={`truncate text-base font-bold ${heading}`}>
                  {item.ingredient.name}
                  {item.matchedAlias && (
                    <span className={`ml-1.5 text-sm font-semibold ${"text-emerald-600"}`}>
                      ({item.matchedAlias})
                    </span>
                  )}
                </p>
